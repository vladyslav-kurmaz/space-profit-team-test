...
function formSubmit() {
    ...
    form.addEventListener('submit', (e) => {
      ...
      .then(res => res.json())
      .then(res => {
        notificationAftersendForm(form, 'done-message', 'Ваша заявка прийнята!')
    })
      .catch(() => {
        notificationAftersendForm(form, 'reject-message', 'Щось пішло не так. Спробуйте ще!')
      })
      .finally(() => {
        form.reset();
...
// Не працює кнопка скролінгу в низ чату
// Кнопка "Показати ще" в коментарях не підвантажує нові коментарі
// Прописувати стилі через теги а не класи
// на панелі зліва не коректно відображаються написи
// велика вкладеність селекторів більше трьох